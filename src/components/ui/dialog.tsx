  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        className={cn(
          "fixed left-1/2 top-1/2 z-50 grid max-h-[92dvh] w-[calc(100vw-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-2xl border border-line bg-surface shadow-2xl focus:outline-none",
          className,
        )}
        {...props}
      >
...
export function DialogHeader({
  ...
  return (
    <div
      className={cn("border-b border-line px-6 py-4 pr-14", className)}
      {...props}
    />
  );
}
...
export function DialogFooter({
  ...
  return (
    <div
      className={cn(
        "flex flex-col-reverse gap-2 border-t border-line bg-surface-muted/50 px-6 py-4 sm:flex-row sm:justify-end",
        className,
      )}
      {...props}
    />
