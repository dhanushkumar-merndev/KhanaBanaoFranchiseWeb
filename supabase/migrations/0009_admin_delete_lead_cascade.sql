-- Permanently remove one lead and all database records tied to it. PostgreSQL
-- cascades the normal child tables; this function also removes the email and
-- audit rows that deliberately use SET NULL/no foreign key. Storage objects
-- are returned to the server action for deletion through the Storage API.
...
begin
  ...
  -- These tables intentionally retain records when a normal lead is removed;
  -- permanent deletion explicitly removes them before deleting the lead.
  ...
  where metadata ->> 'lead_id' = target_lead::text
     or entity_id in (
       ...
       union all
         select rv.id
         from public.document_reviews rv
       ...
       union all
         ...
         from public.training_records t
         join public.franchises f on f.id = t.franchise_id
         where f.lead_id = target_lead
       union all
         select s.id
         from public.setup_items s
...
revoke all on function public.admin_delete_lead_cascade(uuid) from public, anon, authenticated;
